...
In R random samples from the exponential distribution can be obtained using `rexp` e.g. make 10 draws using `rexp(n = 10, rate = 2)`.
The other functions are `pexp` fore the cumulative distribution, `qexp` is the quantile function and density is `dexp`.

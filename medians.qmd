...
calc_medians <- function(fit, NSIM = 10) {
  ...
  for (i in dist_names) {
    ...
    upp_sim[i] <- surv_median(simdat, "upp")
    low_sim[i] <- surv_median(simdat, "low")
  }
  ...
  for (i in dist_names) {
    # flexsurv
    sboot <- normboot.flexsurvreg(fit$models[[i]], B = NSIM)
    para_sample <- asplit(xx, 1)
    
    # stan/hcm
    para_sample <- rstan::extract(fit$models[[i]])
    
    ## mean, upper and lower and/or all samples?
    
    # median_fn <- fit$models$Exponential$dfns$q(0.5, alpha = para_sample, lower.tail = FALSE)
    med_direct[[i]] <- purrr::map_dbl(para_sample, do.call(median_fn(i), .x))
  }

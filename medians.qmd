...
IDEA:
this feels like posterior predictive checking thats already in rstan?
so what if, rather than sample parameters and fit curve for a given distribution, we sample parameter then sample cohort survival times then fit KM then obtain medians from this?
This then would contain the individual level variability as well as the parameter uncertainty...
